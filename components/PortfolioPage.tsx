...
/* Delay before draggables start moving to final coords (must match intro effect below) */
const INTRO_HOLD_MS = 2500;
/* Duration of left/top/transform transition to final spot — keep in sync with CSS 0.7s on draggables */
const INTRO_TO_POSITION_MS = 700;
...
export function PortfolioPage() {
  ...
  const [draggablesReady, setDraggablesReady] = useState(false);
  ...
  useEffect(() => {
    const id = setTimeout(() => setIntroComplete(true), INTRO_HOLD_MS);
    return () => clearTimeout(id);
  }, []);

  /* Wiggle + drag only after the 0.7s move transition has finished */
  useEffect(() => {
    if (!introComplete) return;
    const id = setTimeout(() => setDraggablesReady(true), INTRO_TO_POSITION_MS);
    return () => clearTimeout(id);
  }, [introComplete]);
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            cursor: draggablesReady ? (isDraggingLetter ? "grabbing" : "grab") : "default",
            zIndex: 150,
            transition: isDraggingLetter ? "none" : "left 0.7s ease-out, top 0.7s ease-out, transform 0.7s ease-out",
          }}
          /* Capture: runs before child <a> so native link-drag doesn’t steal the gesture (patches have no link) */
          onMouseDownCapture={(e) => {
            if (!draggablesReady || e.button !== 0) return;
            e.preventDefault();
          ...
          onMouseEnter={() => draggablesReady && setHoveredLetter(true)}
        ...
        >
          ...
          >
            <div
              style={{
                animation:
                  draggablesReady && hoveredLetter && !isDraggingLetter
          ...
          return (
            <div
              ...
              style={{
                ...
                cursor: draggablesReady ? (isDragging ? "grabbing" : "grab") : "default",
                zIndex: 155,
              ...
              onMouseEnter={() => draggablesReady && setHoveredPatchId(patch.id)}
            ...
            >
              ...
              >
                <div
                  style={{
                    animation:
                      draggablesReady && isHovered && !isDragging
                        ? "wiggle 0.5s ease-in-out 1"
                        : "none",
                  }}
                >
